...
phases:
  ...
  build:
    commands:
      ...
      - terraform init -backend=true -backend-config="bucket=ra-test-prod-terraform-state" -backend-config="key=${var.terraform_state_file}" -backend-config="region=${var.region}" -no-color
